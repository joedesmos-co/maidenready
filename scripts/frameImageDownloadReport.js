...
const FRAME_CATEGORY_KEY = "frame";

const scriptDir = dirname(fileURLToPath(import.meta.url));
const projectRoot = join(scriptDir, "..");
const publicRoot = join(projectRoot, "public");
const manifestPath = join(projectRoot, "docs/frame-image-download-manifest.json");
const reportPath = join(projectRoot, "docs/FRAME_IMAGE_DOWNLOAD_REPORT.md");

const allSources = [...presetPartImageSources, ...fiveInchPartImageSources].filter(
  (entry) => entry.categoryKey === FRAME_CATEGORY_KEY,
);
...
export const FAMILY_VARIANT_PART_IDS = new Set([
  "geprc-mark5",
  "iflight-nazgul5-v3-frame",
  "iflight-xl5-v6",
  "happymodel-mobula7-v2-frame",
]);

export { allSources as frameImageSources };
...
export function isWrongFrameImageUrl(imageUrl) {
  if (!imageUrl) {
    return false;
  }
...
export function recommendCandidate(record) {
  const { confidence, status, imageUrl, appearance, sourceType } = record;
  ...
  if (isWrongFrameImageUrl(imageUrl)) {
    return "remove";
  }
...
export function createCandidateRecord({
  ...
  const familyNote = FAMILY_VARIANT_PART_IDS.has(partId)
    ? "Official manufacturer image covers this frame family; catalog SKU maps to a listed size or geometry variant."
    : null;
...
export function cleanupDisallowedLocalImages(manifest = loadDownloadManifest()) {
  ...
  for (const sourceEntry of allSources) {
    ...
    if (downloadedEntry.recommendation !== "keep for review") {
      ...
      if (isWrongFrameImageUrl(downloadedEntry.imageUrl)) {
        downloadedEntry.notes = [
          downloadedEntry.notes,
          "Rejected after review: image URL matches full-drone, wrong-size, accessory, or non-frame-kit pattern.",
        ]
          .filter(Boolean)
          .join(" ");
      }
